  ...
  class CNormalStrategy < BaseStrategy
    ...
    def initialize(a, b, include_rect: nil, exclude_rect: nil)
      ...
      self.regions = [
        ...
        Rectangle.new(bounds.left, exclude_rect.bot, bounds.right, bounds.bot), # rubocop:disable Style/TrailingCommaInArrayLiteral
      ].filter { |r| !r.empty? }
    end

    def score
      result.length.to_f / area
    end

    def area
      return bounds.area if exclude_rect.nil?
      bounds.area - exclude_rect.area
    end
  end
end
