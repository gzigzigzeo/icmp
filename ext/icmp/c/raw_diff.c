...
void raw_diff_compare_region(VALUE region, int width, VALUE a_pixels, VALUE b_pixels, VALUE diff) {
    ...
    for (int y = top; y <= bottom; y++) {
        ...
        for (int x = left; x < right; x++) {
            if (*pixel_a != *pixel_b) {
                ...
                VALUE diff_value = rb_ary_new_from_values(4, r);
                rb_ary_push(diff, diff_value);
            }
...
VALUE raw_diff_compare(VALUE self) {
    ...
    for (int n = 0; n < rb_array_len(regions); n++) {
        ...
        raw_diff_compare_region(region, width, a_pixels, b_pixels, diff);
    }
